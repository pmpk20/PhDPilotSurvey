...
CEWTP  = function(Model){
  CEmodel4 <- CEmodel4IDZC # Enter requisite model here
...
randcoeff <- CEWTP(CEmodelMNLTrunc)
...
apollo_probabilities=function(apollo_beta, apollo_inputs, functionality="estimate"){
  ...
  op_settings3 = list(outcomeOrdered = Q15ThreatToEnvironment, 
                      ...
                      rows           = (Task==1),
                      componentName  = "indic_Q15")
  P[["indic_Q13"]] = apollo_ol(op_settings1, functionality)
  P[["indic_Q14"]] = apollo_ol(op_settings2, functionality)
  P[["indic_Q15"]] = apollo_ol(op_settings3, functionality)
...
CEmodel4OL = apollo_estimate(apollo_beta, apollo_fixed, apollo_probabilities, apollo_inputs)
apollo_modelOutput(CEmodel4OL,modelOutput_settings = list(printPVal=TRUE))
saveRDS("CEmodel4OL.rds")
xtable::xtable(round(data.frame(apollo_modelOutput(CEmodel4OL,modelOutput_settings = list(printPVal=TRUE))),3),digits=3)
CEWTP(CEmodel4OL)


#### CE ICLV Income Dummy: #### 


#### CE ICLV 2 ICLVFULL ID ####
...
### Set core controls
apollo_control = list(
  modelName  = "ICLV modelID",
  modelDescr = "ICLV modelID",
  indivID    = "ID",
  mixing     = TRUE,
  nCores     = 4
)
...
apollo_beta = c(mu_Price    =-3,
                ...
                zeta_Q14   = 1, 
                zeta_Q15   = 1, 
                sigma_Q13=1,sigma_Q14=1,sigma_Q15=1)
...
apollo_randCoeff=function(apollo_beta, apollo_inputs){
  ...
  randcoeff[["LV"]] = gamma_Age*Age +gamma_Gender*Q1Gender + gamma_Distance*Distance + gamma_Income*IncomeDummy + gamma_Employment*Employment + gamma_Experts*Experts + gamma_Cons*Consequentiality + gamma_BP*BP + gamma_Charity*Charity + gamma_Certainty*Q12CECertainty + eta
  randcoeff[["b_Price"]] =  -exp(mu_Price + sig_Price * draws_Price )
  randcoeff[["b_Performance"]] =  -exp(mu_Performance + sig_Performance * draws_Performance )
  randcoeff[["b_Emission"]] =  -exp(mu_Emission + sig_Emission * draws_Emission )
...
apollo_probabilities=function(apollo_beta, apollo_inputs, functionality="estimate"){
  ...
  ### Likelihood of indicators
  op_settings1 = list(outcomeNormal = Q13CurrentThreatToSelf,
                                                         xNormal = zeta_Q13*LV ,
                                                         mu = 0 ,
                                                         sigma = sigma_Q13 ,
                                                         rows = (Task==1) ,
  ...
  op_settings2 = list(outcomeNormal = Q14FutureThreatToSelf,
                                 xNormal = zeta_Q14*LV ,
                                 mu = 0 ,
                                 sigma = sigma_Q14 ,
                                 rows = (Task==1) ,
                                 componentName = "indic_Q14" )
  op_settings3 = list(outcomeNormal = Q15ThreatToEnvironment,
                                 xNormal = zeta_Q15*LV ,
                                 mu = 0,
                                 sigma = sigma_Q15 ,
                                 rows = (Task==1) ,
  ...
  P[["indic_Q13"]] = apollo_normalDensity(op_settings1, functionality)
  P[["indic_Q14"]] = apollo_normalDensity(op_settings2, functionality)
  P[["indic_Q15"]] = apollo_normalDensity(op_settings3, functionality)
...
### Estimate model
CEmodel2IDZC = apollo_estimate(apollo_beta, apollo_fixed, apollo_probabilities, apollo_inputs)
apollo_modelOutput(CEmodel2IDZC,modelOutput_settings = list(printPVal=TRUE))
apollo_saveOutput(CEmodel2IDZC)
saveRDS(CEmodel2IDZC, file="CEmodel2IDZC.rds")


#### CEModel4: ICLVTRUNC ID ####
...
CEmodel4IDZC = apollo_estimate(apollo_beta, apollo_fixed, apollo_probabilities, apollo_inputs)
apollo_modelOutput(CEmodel4IDZC,modelOutput_settings = list(printPVal=TRUE))
apollo_saveOutput(CEmodel4IDZC)
saveRDS(CEmodel4IDZC, file="CEmodel4IDZC.rds")
CEWTP(CEmodel4IDZC)
...
### Set core controls
apollo_control = list(
  modelName  = "ICLV modelSD",
  modelDescr = "ICLV modelSD",
  indivID    = "ID",
  mixing     = TRUE,
  nCores     = 4
)
...
apollo_randCoeff=function(apollo_beta, apollo_inputs){
  ...
  randcoeff[["LV"]] = gamma_Age*Age +gamma_Gender*Q1Gender + gamma_Distance*Distance + gamma_Income*Income+ gamma_Employment*Employment + gamma_Experts*Experts + gamma_BP*BP + gamma_Charity*Charity + eta
  randcoeff[["b_Price"]] =  -exp(mu_Price + sig_Price * draws_Price )
  randcoeff[["b_Performance"]] =  -exp(mu_Performance + sig_Performance * draws_Performance )
  randcoeff[["b_Emission"]] =  -exp(mu_Emission + sig_Emission * draws_Emission )
...
### Estimate model
CEmodel2SD = apollo_estimate(apollo_beta, apollo_fixed, apollo_probabilities, apollo_inputs)
apollo_modelOutput(CEmodel2SD,modelOutput_settings = list(printPVal=TRUE))
apollo_saveOutput(CEmodel2SD)
saveRDS(CEmodel2SD, file="CEmodel2SD.rds")


#### CEModel4: ICLVTRUNC SD ####
...
apollo_control = list(
  modelName  = "ICLVTruncSD",
  modelDescr = "ICLVTruncSD",
  indivID    = "ID", mixing     = TRUE, nCores     = 4)
...
apollo_randCoeff=function(apollo_beta, apollo_inputs){
  randcoeff = list()
  randcoeff[["LV"]] = gamma_Age*Age +gamma_Gender*Q1Gender + gamma_Distance*Distance + gamma_Income + gamma_Experts*Experts + gamma_BP*BP + gamma_Charity*Charity + eta
  randcoeff[["b_Price"]] =  -exp(mu_Price + sig_Price * draws_Price )
  randcoeff[["b_Performance"]] =  -exp(mu_Performance + sig_Performance * draws_Performance )
  randcoeff[["b_Emission"]] =  -exp(mu_Emission + sig_Emission * draws_Emission )
  randcoeff[["asc_B"]] = asc_BB
...
CEmodel4SD = apollo_estimate(apollo_beta, apollo_fixed, apollo_probabilities, apollo_inputs)
apollo_modelOutput(CEmodel4SD,modelOutput_settings = list(printPVal=TRUE))
apollo_saveOutput(CEmodel4SD)
saveRDS(CEmodel4SD, file="CEmodel4SD.rds")
...
### Set core controls
apollo_control = list(
  modelName  = "ICLV modelIDSD",
  modelDescr = "ICLV modelIDSD",
  indivID    = "ID",
  mixing     = TRUE,
  nCores     = 4
)
...
apollo_randCoeff=function(apollo_beta, apollo_inputs){
  ...
  randcoeff[["LV"]] = gamma_Age*Age +gamma_Gender*Q1Gender + gamma_Distance*Distance + gamma_Income*IncomeDummy + gamma_Employment*Employment + gamma_Experts*Experts + gamma_BP*BP + gamma_Charity*Charity + eta
  randcoeff[["b_Price"]] =  -exp(mu_Price + sig_Price * draws_Price )
  randcoeff[["b_Performance"]] =  -exp(mu_Performance + sig_Performance * draws_Performance )
  randcoeff[["b_Emission"]] =  -exp(mu_Emission + sig_Emission * draws_Emission )
...
### Estimate model
CEmodel2IDSD = apollo_estimate(apollo_beta, apollo_fixed, apollo_probabilities, apollo_inputs)
apollo_modelOutput(CEmodel2IDSD,modelOutput_settings = list(printPVal=TRUE))
apollo_saveOutput(CEmodel2IDSD)
saveRDS(CEmodel2IDSD, file="CEmodel2IDSD.rds")


#### CEModel4: ICLVTRUNC IDSD ####
...
apollo_control = list(
  modelName  = "ICLVTruncIDSD",
  modelDescr = "ICLVTruncIDSD",
  indivID    = "ID", mixing     = TRUE, nCores     = 4)
...
CEmodel4IDSD = apollo_estimate(apollo_beta, apollo_fixed, apollo_probabilities, apollo_inputs)
apollo_modelOutput(CEmodel4IDSD,modelOutput_settings = list(printPVal=TRUE))
apollo_saveOutput(CEmodel4IDSD)
saveRDS(CEmodel4IDSD, file="CEmodel4IDSD.rds")
CEWTP(CEmodel4IDSD)